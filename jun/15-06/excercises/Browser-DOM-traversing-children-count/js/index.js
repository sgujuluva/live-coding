...
for (let i = 0; i < wrapperChildren.length; i++) {
  console.log(wrapperChildren[i].children);
  console.log(wrapperChildren[i].children.length);
...
// * Next, we need to find which from the children of
// the element with id `wrapper` has the highest number of classes.

//     For example, if one element has 2 classes
// and another has 3 classes - the one with 3
// classes has the highest number of classes.

//     Let's write a function that returns an object
// that contains the element with the most classes
// and the total number of classes.

function getElementWithMostClasses(list) {
  let element = null;
  let highestClassCount = 0;

  for (let i = 0; i < list.length; i++) {
    if (list[i].classList.length > highestClassCount) {
      element = list[i];
      highestClassCount = element.classList.length;
    }

    return { element, highestClassCount };
  }
}

getElementWithMostClasses(wrapperChildren);
